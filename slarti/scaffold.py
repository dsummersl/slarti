...
- `model/arch/*.c4` — LikeC4 sources
- `model/schema/*.yaml` — LinkML schema
- `model/shapes/*.ttl` — hand-written SHACL
- `model/constraints.yaml` — the rule registry
- `model/data/valid/*`, `model/data/invalid/*` — fixtures
...
Every rule stated in the prose belongs in `model/constraints.yaml`, either with an
...
FILES: dict[str, str] = {
    "slarti.toml": CONFIG,
    "AGENTS.md": AGENTS,
    "model/arch/example.c4": ARCH,
    "model/schema/example.yaml": SCHEMA,
    "model/constraints.yaml": CONSTRAINTS,
    "docs/architecture.md": DOCUMENT,
    ".github/workflows/architecture.yml": WORKFLOW,
}

DIRECTORIES = ("model/shapes", "model/data/valid", "model/data/invalid", "docs/diagrams")
